...
async function main() {
  ...
  // 检查 Key:支持 AMAP_KEY / AMAP_WEBSERVICE_KEY 环境变量,或 config.json
  let __hasKey = process.env.AMAP_KEY || process.env.AMAP_WEBSERVICE_KEY;
  if (!__hasKey) {
    try {
      const fs = require('fs');
      ...
      if (fs.existsSync(cfgPath)) {
        ...
        if (cfg && cfg.webServiceKey && cfg.webServiceKey !== 'your_amap_webservice_key_here') {
          __hasKey = cfg.webServiceKey;
        }
      }
    } catch (e) { /* ignore */ }
  }
  if (!__hasKey) {
    console.error('❌ 未找到高德 Key');
    console.log('\n请用以下任一方式配置:');
    console.log('  export AMAP_WEBSERVICE_KEY=your_key   (或 AMAP_KEY)');
    console.log('  或在 skill 目录写入 config.json: { "webServiceKey": "your_key" }');
    process.exit(1);
  }
