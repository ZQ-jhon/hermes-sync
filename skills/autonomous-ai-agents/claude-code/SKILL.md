...
## PR Creation Pattern (Claude Code as committer)

When asked to have Claude Code *open* a PR from the current branch, use print mode with a tightly-scoped `--allowedTools` whitelist and an explicit "do not push / do not modify code" instruction. The branch should already be pushed to the remote.

```
terminal(command="claude -p 'Create a GitHub PR from the current branch targeting master using the gh CLI.\n1. Run git log master..HEAD --oneline to see what changed.\n2. Write a clear PR title and descriptive body.\n3. gh pr create --base master --head <branch> --title \"...\" --body \"...\"\n4. gh pr view --json url,number,state and print the URL.\nDo NOT push, force-push, or modify any code. Only create the PR.' --allowedTools 'Bash(git log *),Bash(git status),Bash(gh pr create *),Bash(gh pr view *),Bash(gh pr list *)' --max-turns 15", workdir="/path/to/repo", timeout=180)
```

Key points:
- **Scope the tools.** Whitelist only `git log`/`git status` (read) + `gh pr create`/`gh pr view`/`gh pr list`. This prevents the agent from touching code or pushing while still letting it open the PR.
- **Spell out the steps and the prohibitions** in the prompt — slash commands like `/commit` don't exist in `-p` mode, so describe the gh workflow in natural language.
- **`gh` must be authenticated** (`gh auth status`) with `repo` scope, separately from Claude Code's own auth. Check both before delegating.
- **If the target language matters** (e.g. user wants a Chinese PR body), say so explicitly in the prompt — the agent defaults to English otherwise.

### MANDATORY: independently verify the PR after the CLI self-reports

Claude Code's "PR created successfully" output is a **self-report, not proof**. Always verify with a separate `gh` call before telling the user it's done:
```
terminal(command="gh pr view <num> --json number,title,state,url,baseRefName,headRefName,author,commits", workdir="/path/to/repo")
```
Confirm the number, state=OPEN, and that `baseRefName <- headRefName` is the direction you intended. Only then report success with the real URL.
